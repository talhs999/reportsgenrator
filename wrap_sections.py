...
# I want to strip out the sections entirely:
# Stolen Vehicle Check (stolen)
# Outstanding Finance Check (finance)
# Insurance Write-Off Check (writeoff)
# Valuation Estimate (valuation)

# Wrap them in {% if stolen %} etc. so they can be hidden.

def wrap_section(html, keyword):
    ...
    if end_idx != -1:
        # Wrap it!
        var_name = keyword.split(' ')[0].lower()
        if var_name == "insurance" and "write" in keyword.lower():
            var_name = "writeoff"
        elif var_name == "outstanding":
            var_name = "finance"
...
text = wrap_section(text, "Stolen Vehicle Check")
text = wrap_section(text, "Outstanding Finance Check")
text = wrap_section(text, "Insurance Write-Off Check")
text = wrap_section(text, "Valuation Estimate")
